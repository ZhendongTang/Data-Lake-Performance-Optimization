...
case class PreprocessTableUpsert(sqlConf: SQLConf)
  ...
  def apply(upsert: LakeSoulUpsert): UpsertCommand = {
    val LakeSoulUpsert(target, source, condition, migratedSchema, userDefinedTimestamp) = upsert
    ...
    println("userDefinedTimestamp:",userDefinedTimestamp)

    UpsertCommand(source, target, snapshotManagement, condition, migratedSchema, userDefinedTimestamp)
  }
}
